...
SELECT PLAYER_NAME||'선수' 이름, CONCAT(NVL(HEIGHT,0),'cm')키,CONCAT(NVL(WEIGHT,0),'kg') 몸무게
FROM PLAYER
WHERE TEAM_ID LIKE 'K02'
...
SELECT T.TEAM_NAME 팀명, S.STADIUM_NAME 스타디움, SC.AWAYTEAM_ID 원정팀ID, SC.SCHE_DATE 스케쥴날짜,
(SELECT TEAM_NAME FROM TEAM WHERE TEAM_ID LIKE SC.AWAYTEAM_ID) 원정팀
FROM TEAM T JOIN STADIUM S
    ON T.STADIUM_ID = S.STADIUM_ID
    JOIN SCHEDULE SC
...
SELECT 
    ...
    S.STADIUM_NAME 스타디움, SC.SCHE_DATE 스케쥴날짜
FROM TEAM T 
    JOIN PLAYER P
...
SELECT 
    S.STADIUM_NAME 스타디음, 
    SC.SCHE_DATE 경기날짜,
    (SELECT CONCAT(CONCAT(T.REGION_NAME,' '),T.TEAM_NAME) 팀명
        FROM TEAM T 
...
SELECT S.STADIUM_ID, S.STADIUM_NAME 경기장명
FROM STADIUM S
WHERE EXISTS (SELECT 1
            FROM SCHEDULE X
            WHERE X.STADIUM_ID LIKE S.STADIUM_ID
            AND X.SCHE_DATE LIKE '201205%')
ORDER BY 1
;
...
SELECT 
    ROUND(AVG(CASE POSITION WHEN 'MF' THEN HEIGHT  END),2) 미드필더,
    ROUND(AVG(CASE POSITION WHEN 'FW' THEN HEIGHT  END),2) 포워드,
    ROUND(AVG(CASE POSITION WHEN 'DF' THEN HEIGHT  END),2) 디펜더,
    ROUND(AVG(CASE POSITION WHEN 'GK' THEN HEIGHT  END),2) 골기퍼,
...
SELECT P.TEAM_ID 팀아이디,T.TEAM_NAME 팀명, P.PLAYER_NAME 선수, P.POSITION 포지션
FROM PLAYER P JOIN TEAM T
    ON P.TEAM_ID = T.TEAM_ID
WHERE HEIGHT LIKE (SELECT MIN(HEIGHT)
                    FROM PLAYER X
...
SELECT SC.SCHE_DATE,(SELECT T.TEAM_NAME
                    FROM TEAM T
                    WHERE T.TEAM_ID LIKE SC.HOMETEAM_ID)||'-'||
                    (SELECT T.TEAM_NAME
                    FROM TEAM T
                    WHERE T.TEAM_ID LIKE SC.AWAYTEAM_ID) 팀들, 
    SC.HOME_SCORE||'-'||SC.AWAY_SCORE 점수,
...
SELECT 
    ...
    ABS(HOME_SCORE - AWAY_SCORE) 점수차
FROM 
    (SELECT STADIUM_ID,HOME_SCORE,AWAY_SCORE,HOMETEAM_ID,AWAYTEAM_ID,SCHE_DATE
    FROM SCHEDULE 
    WHERE SCHE_DATE LIKE '201208%') SC 
    JOIN STADIUM ST
        ON SC.STADIUM_ID LIKE ST.STADIUM_ID
    JOIN TEAM HT
        ON SC.HOMETEAM_ID LIKE HT.TEAM_ID
...
FROM STADIUM S 
    ...
    (SELECT STADIUM_ID,SCHE_DATE
        FROM SCHEDULE
        WHERE SCHE_DATE BETWEEN '20120501' AND '20120602') SC
...
SELECT T.TEAM_NAME 팀명, P.PLAYER_NAME 선수명, P.HEIGHT 키,ROUND((SELECT AVG(X.HEIGHT)
    FROM PLAYER X
    WHERE X.TEAM_ID LIKE P.TEAM_ID),2)평균키 
FROM PLAYER P JOIN TEAM T
    ON P.TEAM_ID = T.TEAM_ID
ORDER BY 4 DESC
;
...
FROM (SELECT 
        ...
        FROM PLAYER P 
            JOIN TEAM T
                ON P.TEAM_ID = T.TEAM_ID) A   
ORDER BY 4 DESC
;

SELECT 
    DISTINCT STADIUM_ID 스타디움코드, 
    STADIUM_NAME 경기장명
FROM STADIUM  
    JOIN 
    (SELECT STADIUM_ID,SCHE_DATE
